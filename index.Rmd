...
chlsurvey <- methodID %>% group_by(surveyType) %>% summarize(chl=mean(ChlValues), error=se(ChlValues)*1.96, n=length(ChlValues)) %>% filter(!surveyType %in% c("not described","undescribed")) 


plot1 <- ggplot(chlsurvey, aes(x=surveyType, y=chl))+ geom_bar(stat="identity", fill="#9ACC7C", color="black") + geom_errorbar(aes(x=surveyType,  ymin=chl-error, ymax=chl+error), lwd=1, width=0.2)  + xlab("") + ylab("Chlorophyll a (mg / L)")+theme(axis.text=element_text(size=12),
...
## compare methods
methodID %>% group_by(Method) %>% summarize(n=length(ChlValues)/nrow(data)*100) %>% top_n(8)
